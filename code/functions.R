...
createWinPred <- function(master_df, metric, by=NULL, n_min=5, min_diff=NULL) {
  ...
  if (metric=='site')
    if (!is.null(min_diff)) stop('min_diff cannot be specified with site metric.')
  ...
  if (is.null(by)) {
    ...
    ## predict that favored team will win
      ...
      if (is.null(min_diff)) {
        ...
        pred <- ifelse(master_df$line <= -abs(min_diff), TRUE, 
                       ifelse(master_df$line >= abs(min_diff), FALSE, NA))
      }
    }
    ...
    ## against the other team will win
      ...
      if (is.null(min_diff))   {
        ...
        pred <- ifelse(master_df$mtch_mrgn >= abs(min_diff), TRUE, 
                       ifelse(master_df$mtch_mrgn <= -abs(min_diff), FALSE, NA))
      }
    } 
    ...
    ## predict that whichever team with higher metric will win
      ...
      if (is.null(min_diff)) {
        ...
        pred <- ifelse(master_df[[metric]] - master_df[[o_metric]] >= abs(min_diff), TRUE,
                       ifelse(master_df[[metric]] - master_df[[o_metric]] <= -abs(min_diff), FALSE, NA))
      }
    } 
  ...
  ## make variable-specific (e.g. cnf-specific) predictions
    ...
    for (i in 1:nrow(var_df)) {
      ...
      if (is.null(min_diff)) {
        ...
        pred[ind] <- ifelse(master_df[[metric_col]] - master_df[[o_metric_col]] >= abs(min_diff), TRUE,
                            ifelse(master_df[[metric_col]] - master_df[[o_metric_col]] <= -abs(min_diff) , FALSE, NA))[ind]
      }
    }
  }    
...
createPredByVote <- function(obj, maj_vote_cnt) {
  ...
  w_pred_vote_cnts <- Reduce('+', lapply(obj, is.true))

  ## get a vector of loss prediction vote counts
  l_pred_vote_cnts <- Reduce('+', lapply(obj, is.false))
...
createWinPredAccDf <- function(master_df, 
                               metrics=metrics <- c('site', 'line', 'mtch_mrgn', 'rst', 'j', 'wPc'), 
                               n_min=c(5, 10)) {
...
createVarSpWinPredAccDf <- function(master_df, 
                                    metric='wPc', 
  ...
  for (i in seq_along(n_min)) {
    ...
    for (j in seq_along(by_lst)) {
      ...
      ## create prediction
      pred <- createWinPred(master_df=master_df, metric=metric, by=by, n_min=thres)
