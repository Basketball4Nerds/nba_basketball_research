...
####
library(dplyr)
source('helper/createQtrOtPtsDf.R')
source('helper/addQtrOtPtsCols.R')
...
## fill in numbers of defensive rebounds when missing
indices <- is.na(master$dRb)
master$dRb[indices] <- master$rb[indices] - master$oRb[indices]
...
## create backup
write.csv(master, '../../data/processed/regular_season_games_master.csv', row.names=FALSE)
# master <- read.csv('data/processed/regular_season_games_master.csv', stringsAsFactors=FALSE)
